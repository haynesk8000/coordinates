...
describe('interactive physics topic modules', () => {
  it('updates projectile measurements and provides guided challenge progress', () => {
    render(<App />);
    ...
    expect(activePanel().getByLabelText('Guided challenge progress')).toBeVisible();
  });
  ...
  it('supports motion presets and immediate quiz feedback', () => {
    ...
    fireEvent.click(activePanel().getByRole('button', { name: /Turn around/ }));
    expect(activePanel().getByText('Discovery complete')).toBeVisible();
  ...
  it('exposes reference-frame and circular-motion controls', () => {
    ...
    fireEvent.click(activePanel().getByRole('button', { name: /Reverse the orbit/ }));
    expect(activePanel().getByText('Discovery complete')).toBeVisible();
